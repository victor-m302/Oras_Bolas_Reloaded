...
/**
 * 1) recebe os pontos do robô e da bola
 * 2) analisa os pontos e gera o ponto C
 * 3) fecha o triângulo 
 * 4) joga na função calcular3angulos()
 * 5) sai 3 angulos em graus
 */
class Trangulo{
  constructor(roboX,roboY,bolaX,bolaY){
  ...
  setPontoTemp = (x,y) => { //define 3o ponto
    this.pontoTemp.x = x
    this.pontoTemp.y = y
  }
...
class Robo{
  ...
  velocidadeMaxMUx = (ang) => { //  Vxmax = Vmax * cosΘ
    return this.velocidadeMU  * Math.cos(ang)
  }
...
class Bola{
  constructor(){
    ...
    this.y = 0
    this.raio = 0.0105
    this.aceleracaoX = -0.5222
    this.aceleracaoY = -0.4
  }
  sXY = () => {
    this.x = -0.2611*Math.pow(t,2) + 2.9112*t + 0.6844
    this.y = -0.2*Math.pow(t,2) + 1.8*t + 0.5
  }

  trajectBola(x){
      return 0.0303*Math.pow(x,2) + 0.0878*x + 1.0275
  } //retorna f(x)

  sXBola(t){
      return -0.2611*Math.pow(t,2) + 2.9112*t + 0.6844
  } //retorna X(t)  //Equação do espaço da bola em X:

  sYBola(t){
      return -0.2*Math.pow(t,2) + 1.8*t + 0.5
  } //retorna Y(t)  //Equação do espaço da bola em Y:

  vXBola(t){
    return this.aceleracaoX *t + 2.9112
  } //Vx(t)  // Equação da velocidade da bola em X:

  vYBola(t){
    return aceleracaoY*t + 1.8
  } //Vy(t)  // Equação da velocidade da bola em Y:

  aXBola(t){
    return this.aceleracaoX * t
  }//Ax(t) = -0,5222 m/s²  //Equação da aceleração da bola em X:

  aYBola(t){
    return aceleracaoY * t
  }//Ay(t) = -0.4 m/s²  //Equação da aceleração da bola em Y:
