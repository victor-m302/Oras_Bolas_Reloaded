...
class Layout {
  constructor(firstTitle, xAxisTitle='Tempo(t)', yaxisTitle) {
...
function Envia() {
  ...
  if (C == 0) {
    for (k = 0; k < t; k += 0.02) {
      ...
      if (x > posX[i]) {
        Rx[n] = x - 0.1005 - (3.125 * cosseno * (k * k));
  ...
  if (C == 1) {
    for (j = 0; j < t - 0.4; j += 0.02) {
      ...
      if (x > posX[i]) {
        RVx[n] = (2.5 * cosseno) * -1;
        RAx[n] = (6.25 * cosseno) * -1;
        Rx[n] = (x - 0.1005 - ((3.125 * cosseno) * 0.16)) + RVx[n] * j;
      ...
      if (y > posY[i]) {
        RVy[n] = (2.5 * seno) * -1;
        RAy[n] = (6.25 * seno) * -1;
        Ry[n] = (y - 0.1005 - ((3.125 * seno) * 0.16)) + RVy[n] * j;
...
function posRobo(){
  ...
  let layout2 = new Layout('Posição do Robô X(t) e Y(t)','Tempo(t)','X(t) e Y(t)')
  traceRy.x = Dt;
  traceRy.y = Ry;
  let data = [traceRx, traceRy];
  //console.log(traceRy);
  //console.log(traceRx);
  Plotly.newPlot('myDiv4', data, layout2);
}
...
function speedRobo(){ 
  ...
  let layout3 = new Layout('Velocidade do Robô Vx(t) e Vy(t)','Tempo(t)','Vx(t) e Vy(t)')
  traceRVy.x = Dt;
  traceRVy.y = RVy;
  let data = [traceRVx, traceRVy];
  Plotly.newPlot('myDiv6', data, layout3);
}
...
function acelRobo(){  
  ...
  let layout4 = new Layout('Aceleração do Robô Ax(t) e Ay(t)','Tempo(t)','Ax(t) e Ay(t)')
  let traceRAy=new Tracer('Aceleração Y','#8B1A1A','#8B1A1A')  
  traceRAy.x = Dt;
  traceRAy.y = RAy;
  let data = [traceRAx, traceRAy];
...
function trajBall(){
  ...
  let layout6 = new Layout('Trajetória da Bola Y(x)','Posição X','Posição Y')
  let traceTB2 = new Tracer('Total','#8B1A1A','#8B1A1A')  
  traceTB2.x = Bx; //former tracerTB
  traceTB2.y = By;
  let data = [traceTB2];
...
function trajRobo(){
  ...
  let layout7 = new Layout('Trajetória do Robô','Posição X','Posição Y')
...
function interceptTraj(){
  /*TRAJETÓRIA INTERCEPTAÇÃO*/
  let layout8 = new Layout('Trajetória da Interceptação','Posição X','Posição Y')
  ...
  traceRR.x = Rx;
  traceRR.y = Ry;
  let data = [traceTB2, traceTB, traceRR];
...
function bolaPos(){
  ...
  traceTBX.y = posX;
  /*Posição - Bola(y)*/
  let layout9 = new Layout('Posição da Bola X(t) e Y(t)','Tempo(t)','X(t) e Y(t)')
  let traceTBY = new Tracer('Posição Y','#8B1A1A','#8B1A1A')
  traceTBY.x = Bt;
  traceTBY.y = posY;
  let data = [traceTBX, traceTBY];
...
function speedBall(){
  ...
  let layout10 = new Layout('Velocidade da Bola Vx(t) e Vy(t)','Tempo(t)','Vx(t) e Vy(t)')
  let traceTBVY = new Tracer('Velocidade Y','#8B1A1A','#8B1A1A')
  traceTBVY.x = Bt;
  traceTBVY.y = BVy;
  let data = [traceTBVX, traceTBVY];
...
function acelBola(){
  ...
  let layout11 = new Layout('Aceleração da Bola Ax(t) Ay(t)','Tempo(t)','Ax(t) e Ay(t)')
  traceTBAY.x = Bt;
  traceTBAY.y = BAy;
  let data = [traceTBAX, traceTBAY];
  Plotly.newPlot('myDiv9', data, layout11);
}

function toggleDisplay(number){
  for(i=1;i<11;i++){document.getElementById("myDiv"+i).style.display = 'none';}
  document.getElementById("myDiv"+number).style.display = 'block';
}
...
function canvasMain() {
  ...
  vetx = vet5[0];
  vety = vet5[1];
  //let len_vet = vetx.length;
  //document.getElementById("demo1234").innerHTML = "precisamente o tamanho do vetor len_vet é: " + len_vet;
  //console.log(len_vet);
  let len_vet = vetx.length - 1;

  function azul(a) {
    ctx.beginPath();
    ctx.arc(vetx[a] * 100, vety[a] * 100, 10, 0, Math.PI * 2);
  ...
  function vermelho(b) {
    ctx.beginPath();
    ctx.arc(posX[b] * 100, posY[b] * (100), 10, 0, Math.PI * 2);
